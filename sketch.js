...
function draw() {
  ...
  if(gameState === play) {
    ...
    fill("white");
    textSize(40);
    text("YOU ARE DEAD!",canvas.width/2-400,canvas.height/2);
    text("The asteroids destroyed you.",canvas.width/2-400,canvas.height/2+100);
    text("YOUR FINAL SCORE:"+score,canvas.width/2-400,canvas.height/2+200);
  ...
  if(gameState === instruction) {
    ...
    textSize(50);
    text("DEATH SENTENCE",canvas.width/2-300,canvas.height/2-300);
    text("ENJOY YOUR DEATH...HEHE BOI!",canvas.width/2-300,canvas.height/2+100);
    stroke("yellow");
    fill("yellow");
    textSize(35);
    textFont("times new roman");
    text("year 11111100500  something maybe.....idk",canvas.width/2-300,canvas.height/2-250);
    text(" Some asteroids are coming towords our Earth...But idc",canvas.width/2-300, canvas.height/2 - 210);
    text("  You are the only X-wing Starfighter we have so risk ur life  ",canvas.width/2-500,canvas.height/2-170);
    text("  Help the people and Earth !",canvas.width/2-300,canvas.height/2-130);
    text("  Press 'S' to Shoot.",canvas.width/2-300,canvas.height/2-90);
    text("  Use the arrow keys to move.",canvas.width/2-300,canvas.height/2-50);
    text("  Press 'L' to sacrifise your life.",canvas.width/2-100,canvas.height/2-10);
  ...
  if(frameCount % 110 === 0) {
    ...
    switch(rand) {
      ...
      case 3: asteroid.addImage(asteroid3);
             ...
             break;
      case 4:asteroid.addImage(asteroid3);
      asteroid.setCollider("circle",0,0,170)
             break;
      case 6:asteroid.addImage(asteroid2);
      asteroid.setCollider("circle",0,0,170)
             break;          
      case 5:asteroid.addImage(asteroid1);
      asteroid.setCollider("circle",0,0,170)
           default : break;        
    }
